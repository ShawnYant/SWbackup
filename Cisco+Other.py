...
def backup_switch_config(switch_ip, username, password, port, device_type, output_path):
    try:
        # 创建SSH客户端对象
        ...
        while True:
            if shell.recv_ready():
                ...
                output += data
                if (more_prompt_huawei in data and device_type.lower() == 'huawei') or \
                   (more_prompt_cisco in data and device_type.lower() == 'cisco'):  # 检测到分页提示符
                    shell.send(' ')  # 发送空格以继续显示下一页
                elif (end_prompt_huawei in data and device_type.lower() == 'huawei') or \
                     (end_prompt_cisco in data and device_type.lower() == 'cisco'):  # 假设提示符包含 ']' 或 '#'
                    break
        ...
        print(f'交换机 {switch_ip} ({device_type}) 的配置文件备份完成，保存为 {filename}。')
...
def main():
    ...
    for index, row in df.iterrows():
        ...
        device_type = row['DeviceType']  # 从Excel中读取设备类型
        success, error = backup_switch_config(switch_ip, username, password, port, device_type, output_path)
        if not success:
            failed_switches.append((switch_ip, error))
            print(f'备份交换机 {switch_ip} ({device_type}) 的配置文件失败: {error}')
